...
def my_videos_upload(request):
    with limit(limit_key(request.remote_addr, RateLimit.VIDEO_UPLOAD), limit=10, period='1d', save=False) as diff:
        if diff:
        ...
        return render(request, "my_videos_upload.html", options)
...
def results(request):
    return render(request, "results.html", {
        ...
        ).select_related('author').filter(rank__gte=0.0001).order_by("-rank")[:20]
    })
...
def watch(request):
    video_fields = [
        'author__name', 'author__handle', 'author__avatar',  'author__record_id',   'author__created_at', 
        'created_at', 'file', 'status', 
        'title', 'description', 'views', 'thumbnail', 'duration', 
        'record_id', 'positive_ratings', 'negative_ratings', 'comments_count',
        'allow_comments', 'user_can_vote_on_comments', 'user_can_see_ratings',
        'visibility'
    ]
    ...
    options = {
        ...
        'recommended_videos': Video.objects.filter(
            ...
            'author__name', 'author__record_id', 'author__handle', 'created_at', 
            'title', 'views', 'thumbnail', 'duration', 'record_id',
        ).order_by('-id')[:9]
    }
    if video.user_can_vote_on_comments:
        if video.negative_ratings == 0:
            ...
            options['ratings_ratio'] = (video.positive_ratings / (video.positive_ratings + video.negative_ratings)) * 100
    ...
    if my_profile:
        ...
        options['is_disliked'] = my_profile.pl_dislikes.videos.filter(video=video).exists()
    
    return render(request, "watch.html", options)