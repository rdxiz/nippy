        ...
        if form.is_valid():
            with limits(
                ...
                if diff:
                    ...
                    return render(request, "legacy/profile_switcher.html", options)

            data = form.cleaned_data
            profile = Profile.objects.create(
                name=data["name"], avatar=data.get("avatar"), user=request.user
            )
            request.session[SESSION_KEY_PROFILE] = profile.id
            return redirect("core:index")
        else:
...
def profile_view(request, profile_id=None, profile_handle=None, page=None):
    ...
    if profile.following_count > 0:
        ...
        if request.profile:
            ...
            following_subquery = Follow.objects.filter(
                follower=request.profile, following=OuterRef("following_id")
            ).values("id")

            related_profiles = related_profiles.annotate(
                is_following=Exists(following_subquery)
            )
        related_profiles = related_profiles.all().order_by(
            "-following__followers_count"
        )[:8]
    if not page:
    ...
    return render(request, "legacy/profile.html", options)
